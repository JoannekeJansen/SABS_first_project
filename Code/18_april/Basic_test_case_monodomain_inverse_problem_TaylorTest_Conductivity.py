...
#
# Taylor Test when optimizing for M
#
# ===========================================================
#
...
if __name__ == "__main__":
    M = Constant(0.255)	      	  # initial guess.
    v = forward(M)                  # solve the forward problem once. 
    ...
    dJdnu = compute_gradient(J, Control(M))
    Jnu = assemble(inner(v, v)*dx) # current value

    parameters["adjoint"]["test_derivative"] = True
    parameters["adjoint"]["stop_annotating"] = True # stop registering equations

    def Jhat(M): # the functional as a pure function of M
         v = forward(M)
         return assemble(inner(v, v)*dx)

    conv_rate = taylor_test(Jhat, Control(M), Jnu, dJdnu, seed=(0.01))
