# Record for different values of g_CaL
# ===========================================================
#
...
for fact in np.array([0.5,1.5]):
    print "We multiply g_CaL by", fact
    model = Paci2013_ventricular()
    model.set_parameters(g_CaL=fact*params["g_CaL"])

    if CreateInitial == True:
      # Create HDF5 file to save initial conditions
      hdf_vs = HDF5File(mesh.mpi_comm(), "initial_conditions_g_CaL_%s.h5" % fact, "w")
    ...
    vsinit = Function(Q)
    hdf_vs = HDF5File(mesh.mpi_comm(), "initial_conditions_g_CaL_%s.h5" % fact, "r")
    ...
    # Save data
    np.savetxt("recorded_times_g_CaL_%s.txt" % fact, times)
    np.savetxt("v_left_g_CaL_%s.txt" % fact, v_left)
    np.savetxt("v_right_g_CaL_%s.txt" % fact, v_right)
    np.savetxt("cai_left_g_CaL_%s.txt" % fact, cai_left)
    np.savetxt("cai_right_g_CaL_%s.txt" % fact, cai_right)
