# Record for different values of g_K1
# ===========================================================
#
...
for fact in np.array([0.5,1.5]):
    print "We multiply g_K1 by", fact
    model = Paci2013_ventricular()
    model.set_parameters(g_K1=fact*params["g_K1"])

    if CreateInitial == True:
      # Create HDF5 file to save initial conditions
      hdf_vs = HDF5File(mesh.mpi_comm(), "initial_conditions_g_K1_%s.h5" % fact, "w")
    ...
    vsinit = Function(Q)
    hdf_vs = HDF5File(mesh.mpi_comm(), "initial_conditions_g_K1_%s.h5" % fact, "r")
    ...
    # Save data
    np.savetxt("recorded_times_g_K1_%s.txt" % fact, times)
