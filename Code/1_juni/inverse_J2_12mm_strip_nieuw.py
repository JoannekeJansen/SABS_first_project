...
small_sub_domain = CompiledSubDomain("x[0] > 7.4 && x[0] < 7.6")
small_sub_domain.mark(cell_domains, 1)
...
def forward(factor_gna, factor_gcal, factor_gk1, factor_gkr):
    ...
    # times_obs = np.loadtxt(observed_times)
    ...
    for (timestep, fields) in solver.solve(interval, h):
        ...
        #     print v_obs[i+25]
        #     print "and ", vs((7.5,0.0))[0]
        #     i=i+1

    return v_peak_time, vs, vur
...
if __name__ == "__main__":
    ...
    (v_peak_time, vs, vur) = forward(factor_gna, factor_gcal, factor_gk1, factor_gkr)    # solves the forward problem once.
    ...
    for i in range(start_interval, end_interval):   
        V_obs = project(Constant(v_obs[i+shift]), Q)
        Cai_obs = project(Constant(cai_obs[i+shift]), Q)
        I = I + (inner(split(vs)[0] - V_obs, split(vs)[0] - V_obs)*dx(subdomain_data=cell_domains)*dt[float(times[i])]) \
            /(assemble(inner(V_obs, V_obs)*dx(subdomain_data=cell_domains))) 
        I = I + (inner(split(vs)[16] - Cai_obs, split(vs)[16] - Cai_obs)*dx(subdomain_data=cell_domains)*dt[float(times[i])]) \
            /(assemble(inner(Cai_obs, Cai_obs)*dx(subdomain_data=cell_domains))) 
    J = Functional(I/(N-abs(shift)))
    ...
    if Evaluate_J == True:
        ...
        control_values=np.array([1.0*orig_val,0.9*orig_val])
        for i in range(np.size(control_values)):
            J_values[i]=rf(Constant(control_values[i]))
            print "Control values and J value", control_values[i], J_values[i], v_peak_time2
            np.savetxt('J_values_{0}_{1}_{2}_{3}_12mm_strip_{4}.txt'.format(gnaf,gk1f,gkrf,gcalf,args.control_var), np.column_stack((J_values, control_values)))
