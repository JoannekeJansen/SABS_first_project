...
#Define dummy mesh for storage of solution in single point
dummy_mesh = UnitIntervalMesh(1)
Q0 = VectorFunctionSpace(dummy_mesh, "DG", 0, 18)
#Q00 = VectorFunctionSpace(dummy_mesh, "DG", 0, 1)
vs_single_point = Function(Q0)
#peak_time = Function(Q00)
dx = Measure('dx', domain=dummy_mesh)
...
def forward(factor_gna, factor_gcal, factor_gk1, factor_gkr):
    ...
    for (timestep, fields) in solver.solve(interval, h):
        ...
        (vs_, vs, vur) = fields
        vs_single_point = vs((7.5,0.0))
        #print vs_single_point[0]
        if vs_single_point[0]>v_peak:
            v_peak=vs_single_point[0]
            v_peak_time=timestep[1]

    return v_peak_time, vs
...
def derivative_cb_post(j, dj, m):
    ...
    np.savetxt(f, np.column_stack((j, float(dj[0]), float(dj[1]), float(dj[2]), float(dj[3]), float(m[0]),float(m[1]),float(m[2]),float(m[3]))))
    f.close()
...
if __name__ == "__main__":
    ...
    (v_peak_time, vs) = forward(factor_gna, factor_gcal, factor_gk1, factor_gkr)    # solves the forward problem once.
    ...
    for i in range(start_interval, end_interval):   
        print i
        I = I + (inner((vs((7.5,0.0)))[0] - v_obs[i+shift], (vs((7.5,0.0)))[0] - v_obs[i+shift])*dx*dt[float(times[i])]) \
            /(inner(v_obs[i+shift], v_obs[i+shift]))
        I1 = I1 + (inner((vs((7.5,0.0)))[0] - v_obs[i+shift],(vs((7.5,0.0)))[0] - v_obs[i+shift])*dx*dt[float(times[i])]) \
            /(inner(v_obs[i+shift], v_obs[i+shift]))
        I = I + (inner((vs((7.5,0.0)))[16] - cai_obs[i+shift], (vs((7.5,0.0)))[16] - cai_obs[i+shift])*dx*dt[float(times[i])]) \
            /(inner(cai_obs[i+shift], cai_obs[i+shift]))
        I2 = I2 + (inner((vs((7.5,0.0)))[16] - cai_obs[i+shift], (vs((7.5,0.0)))[16] - cai_obs[i+shift])*dx*dt[float(times[i])]) \
            /(inner(cai_obs[i+shift], cai_obs[i+shift]))
        if (vs((7.5,0.0)))[0]>v_peak2:
            v_peak2=(vs((7.5,0.0)))[0]
            v_peak_time2=times[i]
    J = Functional(I/(N-abs(shift))+inner(peak_time-v_peak_time2,peak_time-v_peak_time2)*dx*dt[FINISH_TIME]/peak_time)
    J1 = Functional(I1/(N-abs(shift)))
    J2 = Functional(I2/(N-abs(shift)))
    J3 = Functional(inner(peak_time-v_peak_time2,peak_time-v_peak_time2)*dx*dt[FINISH_TIME]/peak_time)
    ...
    if Optimize == True:
        ...
        opt_ctrls = minimize(rf, bounds = [[0.9, 0.9, 0.9, 0.9], [1.1, 1.1, 1.1, 1.1]], tol=1e-25, options={"maxiter": 10, "gtol": 1e-25, "ftol": 1e-25})        #opt_ctrls = minimize(rf)
    ...
    if Evaluate_J == True:
        ...
        rf3 = ReducedFunctional(J2, [Control(controls[args.control_var])])
        ...
        for i in range(np.size(control_values)):
            ...
            J3_values[i]=rf3(Constant(control_values[i]))
            print "Control values and J value", control_values[i], J_values[i], peak_time2,peak_time
            np.savetxt('J_values_{0}_{1}_{2}_{3}_12mm_strip_{4}.txt'.format(gnaf,gk1f,gkrf,gcalf,args.control_var), np.column_stack((J_values, J1_values, J2_values, J3_values, control_values)))
