...
def main(model):
  ...
  for (timestep, fields) in solver.solve(interval, h):
      ...
      if True:
        ...
        v_left.append(vs((1.5,0.0))[0])
        v_right.append(vs((2.5,0.0))[0])
        cai_left.append(vs((1.5,0.0))[16])
        cai_right.append(vs((2.5,0.0))[16])
        i=i+1
        #print i
...
if CreateInitial == True:
  ...
  hdf_vs = HDF5File(mesh.mpi_comm(), "initial_conditions_3mm_strip.h5", "w")
...
# Save data
np.savetxt("recorded_times_3mm_strip.txt", times)
np.savetxt("v_left_3mm_strip.txt", v_left)
np.savetxt("v_right_3mm_strip.txt", v_right)
np.savetxt("cai_left_3mm_strip.txt", cai_left)
np.savetxt("cai_right_3mm_strip.txt", cai_right)        
