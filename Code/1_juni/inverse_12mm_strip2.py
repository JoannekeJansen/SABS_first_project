...
if __name__ == "__main__":
    ...
    if Evaluate_J == True:
        ...
        M = 10 # Number of functional evaluations
        J_values = np.zeros(M)
        ...
        control_values=np.array([1.01*orig_val,1.02*orig_val, 1.03*orig_val, 1.04*orig_val,1.05*orig_val,1.06*orig_val,1.07*orig_val, 1.08*orig_val,1.09*orig_val,1.10*orig_val])
        for i in range(np.size(control_values)):
            ...
            np.savetxt('J_values2_{0}_{1}_{2}_{3}_12mm_strip_{4}.txt'.format(gnaf,gk1f,gkrf,gcalf,args.control_var), np.column_stack((J_values, J1_values, J2_values, control_values)))
