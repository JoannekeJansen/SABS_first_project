...
# ===========================================================
#
# This test case solves the monodomain equations on a [0,3]*[0,0.01] mm^2 strip,
# with the Paci2013 cell model, using the splittingsolver. W
#
...
if args.sigma:
  ...
  noise_percentage_sigma_t=5
  noise_percentage_gna=5
  noise_percentage_gk1=5
  noise_percentage_gkr=5
  noise_percentage_gcal=5
...
mesh = RectangleMesh(Point(0.0, 0.0), Point(3.0, 0.01), 50, 1)
...
# Add random noise to tangential conductivity
SD = noise_percentage_sigma_t*0.01*M_t
M_t_noise = Function(Q1)
prng = numpy.random.RandomState()
M_t_noise.vector()[:] = M_t + prng.normal(0, SD, Q1.dim())
...
def main(model):
  ...
  hdf_v = HDF5File(mesh.mpi_comm(), 'observed_v_3mm_strip_{0}_{1}_{2}_{3}_{4}_{5}_{6}.h5'.format('noise_percentage_sigma_t', \
  ...
  hdf_cai = HDF5File(mesh.mpi_comm(), 'observed_cai_3mm_strip_{0}_{1}_{2}_{3}_{4}_{5}_{6}.h5'.format('noise_percentage_sigma_t', \
   'noise_percentage_gna', 'noise_percentage_gk1', 'noise_percentage_gkr', 'noise_percentage_gcal', 'noise_percentage_v', 'noise_percentage_cai'), "w")
  times = []
  i=0
  ...
  # Save time steps
  np.savetxt('observed_times_3mm_strip_{0}_{1}_{2}_{3}_{4}_{5}_{6}.txt'.format('noise_percentage_sigma_t', \
