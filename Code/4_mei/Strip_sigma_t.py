...
def main(sigma_t_factor):
  ...
  sigma_l = 0.15         # mS / mm
  sigma_t = 0.02*sigma_t_factor           # mS / mm
  beta = 200.0            # mm^{-1}
  C_m = 0.2               # mu F / mm^2
...
for fact in np.linspace(0.1,2,20):
    print fact
    (times, v_left, v_right, cai_left, cai_right) = main(fact)
    np.savetxt("recorded_times.txt" % fact, times)
    np.savetxt("v_left_sigma_t_%s.txt" % fact, v_left)
    np.savetxt("v_right_sigma_t_%s.txt" % fact, v_right)
    np.savetxt("cai_left_sigma_t_%s.txt" % fact, cai_left)
    np.savetxt("cai_right_sigma_t_%s.txt" % fact, cai_right)
