...
for fact in np.linspace(0.1,2,20):
    ...
    model.set_parameters(SR_Ca_release_ks_factor=fact)
    (times, v_left, v_right, cai_left, cai_right) = main(model)
    np.savetxt("recorded_times.txt" % fact, times)
    np.savetxt("v_left_SR_Ca_release_ks_%s.txt" % fact, v_left)
    np.savetxt("v_right_SR_Ca_release_ks_%s.txt" % fact, v_right)
    np.savetxt("cai_left_SR_Ca_release_ks_%s.txt" % fact, cai_left)
    np.savetxt("cai_right_SR_Ca_release_ks_%s.txt" % fact, cai_right)
