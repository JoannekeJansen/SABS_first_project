   ...
   controls = {'gna': 1.0, 'gcal': 1.0, 'gk1': 1.0, 'gkr': 1.0} # initial guess
    ...
    for i in range(np.size(control_values)):
      for j in range(np.size(control_values2)):
        ...
        np.savetxt('J_values_{0}_{1}_{2}_{3}_single_cell_{4}_{5}.txt'.format(gnaf,gk1f,gkrf,gcalf,args.control_var,args.control_var2), np.column_stack((J_values, J1_values, J2_values, C, C2)))
        k=k+1
