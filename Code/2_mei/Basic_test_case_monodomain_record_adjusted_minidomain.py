...
for (timestep, fields) in solver.solve(interval, h):
    ...
    #print "v(0.25,0.25) = ", vs((0.25,0.25))[0]
    ...
    if (abs(round(timestep[1])-timestep[1])) < (0.1*h):
      ...
      #v_centre.append(vs((0.25,0.25))[0])
      #v_corner.append(vs((0,0))[0])
      #cai_centre.append(vs((0.25,0.25))[36])
...
# Save time steps
np.savetxt("recorded_times_adjusted_Ca100_mini.txt", times)

# Save v and [Ca]_i at point in centre and point in corner
#np.savetxt("v_dlc_adjusted_Cal100_dm.txt", v_centre)
#np.savetxt("v_trc_adjusted_Cal100_dm.txt", v_corner)
#np.savetxt("cai_dlc_adjusted_Cal100_dm.txt", cai_centre)
#np.savetxt("cai_trc_adjusted_Cal100_dm.txt", cai_corner)
