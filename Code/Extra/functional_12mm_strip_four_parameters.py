...
parser.add_argument("-gna_value", type=float, help="The added percentage of noise to the observed cai")
...
if __name__ == "__main__":
    ...
    controls = {'gna': args.gna_value, 'gcal': 1.0, 'gk1': 1.0, 'gkr': 1.0} # initial guess

    # Compute value of J for different values of the control_var and save to file
    control_values=np.arange(0.90,1.10,0.02)
    control_values2=np.arange(0.90,1.10,0.02)
    control_values3=np.arange(0.90,1.10,0.02)
    M = np.size(control_values)
    M2 = np.size(control_values2)
    M3 = np.size(control_values3)
    J_values = np.zeros(M*M2*M3)
    J1_values = np.zeros(M*M2*M3)
    J2_values = np.zeros(M*M2*M3)
    C = np.zeros(M*M2*M3)
    C2 = np.zeros(M*M2*M3)
    C3 = np.zeros(M*M2*M3)
    C0 = np.zeros(M*M2*M3)
    ...
    for i in range(np.size(control_values)):
      for j in range(np.size(control_values2)):
        for l in range(np.size(control_values3)):
          controls['gcal']=control_values[i]
          controls['gk1']=control_values2[j]
          controls['gkr']=control_values3[l]
          C[k]=controls['gcal']
          C2[k]=controls['gk1']
          C3[k]=controls['gkr']
          C0[k]=controls['gna']
          (J_values[k], J1_values[k], J2_values[k]) = forward(controls['gna'], controls['gcal'], controls['gk1'], controls['gkr']) 
          print "Control values and J value", C0[k], C[k], C2[k], C3[k], J_values[k]
          np.savetxt('J_values_4parameters_{0}_{1}_{2}_{3}_12mm_strip_{4}.txt'.format(gnaf,gk1f,gkrf,gcalf,args.gna_value), np.column_stack((J_values, J1_values, J2_values, C0, C, C2, C3)))
          k=k+1
