...
observed_peak_time ='v_peak_time_{0}_{1}_{2}_{3}_12mm_strip_{4}_{5}_{6}_{7}_{8}_{9}.txt'.format(gnaf,gk1f,gkrf,gcalf, \
 noise_percentage_gna, noise_percentage_gk1, noise_percentage_gkr, noise_percentage_gcal, noise_percentage_v, noise_percentage_cai)
observed_characteristics ='characteristics_observed_data_{0}_{1}_{2}_{3}_12mm_strip_{4}_{5}_{6}_{7}_{8}_{9}.txt'.format(gnaf,gk1f,gkrf,gcalf, \
 noise_percentage_gna, noise_percentage_gk1, noise_percentage_gkr, noise_percentage_gcal, noise_percentage_v, noise_percentage_cai)
...
if __name__ == "__main__":
    ...
    characteristics = np.loadtxt(observed_characteristics)
    ...
    J4_values = np.zeros(M)
    J5_values = np.zeros(M)
    J6_values = np.zeros(M)
    J7_values = np.zeros(M)
    J8_values = np.zeros(M)
    J9_values = np.zeros(M)
    J10_values = np.zeros(M)
    J11_values = np.zeros(M)
    J12_values = np.zeros(M)
    J13_values = np.zeros(M)
    J14_values = np.zeros(M)
    J15_values = np.zeros(M)
    ...
    for k in range(np.size(control_values)):
      ...
      J3_values[k] = abs(peak_time-v_peak_time2)*1000 
      #print "v_peak_time2 = ", v_peak_time2
      #print "v_peak_time= ", peak_time

      # Calculate characteristics
      amplitude_v2=max(v_single_point)-min(v_single_point)
      amplitude_c2=max(cai_single_point)-min(cai_single_point)
      upstroke_velocity_c2= max(cai_single_point[5:(np.argmax(cai_single_point))]-cai_single_point[0:(np.argmax(cai_single_point)-5)])*200   
      upstroke_velocity_v2= max(v_single_point[5:(np.argmax(v_single_point))]-v_single_point[0:(np.argmax(v_single_point)-5)])*200 

      temp=np.argmax(v_single_point>(min(v_single_point)+0.7*amplitude_v2))
      v_30_p2=np.argmax(v_single_point[temp:-1]<(min(v_single_point)+0.7*amplitude_v2))
      temp=np.argmax(cai_single_point>(min(cai_single_point)+0.7*amplitude_c2))
      c_30_p2=np.argmax(cai_single_point[temp:-1]<(min(cai_single_point)+0.7*amplitude_c2))

      temp=np.argmax(v_single_point>(min(v_single_point)+0.5*amplitude_v2))
      v_50_p2=np.argmax(v_single_point[temp:-1]<(min(v_single_point)+0.5*amplitude_v2))
      temp=np.argmax(cai_single_point>(min(cai_single_point)+0.5*amplitude_c2))
      c_50_p2=np.argmax(cai_single_point[temp:-1]<(min(cai_single_point)+0.5*amplitude_c2))

      temp=np.argmax(v_single_point>(min(v_single_point)+0.3*amplitude_v2))
      v_70_p2=np.argmax(v_single_point[temp:-1]<(min(v_single_point)+0.3*amplitude_v2))
      temp=np.argmax(cai_single_point>(min(cai_single_point)+0.3*amplitude_c2))
      c_70_p2=np.argmax(cai_single_point[temp:-1]<(min(cai_single_point)+0.3*amplitude_c2))

      temp=np.argmax(v_single_point>(min(v_single_point)+0.1*amplitude_v2))
      v_90_p2=np.argmax(v_single_point[temp:-1]<(min(v_single_point)+0.1*amplitude_v2))
      temp=np.argmax(cai_single_point>(min(cai_single_point)+0.1*amplitude_c2))
      c_90_p2=np.argmax(cai_single_point[temp:-1]<(min(cai_single_point)+0.1*amplitude_c2))

      J4_values[k] = abs(characteristics[0]-amplitude_v2)
      J5_values[k] = abs(characteristics[1]-amplitude_c2)
      J6_values[k] = abs(characteristics[2]-upstroke_velocity_c2)
      J7_values[k] = abs(characteristics[3]-upstroke_velocity_v2)
      J8_values[k] = abs(characteristics[4]-v_30_p2)
      J9_values[k] = abs(characteristics[5]-c_30_p2)
      J10_values[k] = abs(characteristics[6]-v_50_p2)
      J11_values[k] = abs(characteristics[7]-c_50_p2)
      J12_values[k] = abs(characteristics[8]-v_70_p2)
      J13_values[k] = abs(characteristics[9]-c_70_p2)
      J14_values[k] = abs(characteristics[10]-v_90_p2)
      J15_values[k] = abs(characteristics[11]-c_90_p2)

      print "Control values and J value", control_values[k], J_values[k]
      np.savetxt('J2_values_{0}_{1}_{2}_{3}_12mm_strip_{4}.txt'.format(gnaf,gk1f,gkrf,gcalf,args.control_var), np.column_stack((J_values, J1_values, J2_values, J3_values, \
       J4_values, J5_values, J6_values, J7_values, J8_values, J9_values, J10_values, J11_values, J12_values, J13_values, J14_values, J15_values, control_values)))
