...
cell_model.set_parameters(GNa=1.0*23)
...
hdf_v = HDF5File(mesh.mpi_comm(), "Strip_synthetic_observations_v.h5", "w")
hdf_cai = HDF5File(mesh.mpi_comm(), "Strip_synthetic_observations_cai.h5", "w")
times = []
#v_left = []
#v_right = []
#cai_left = []
...
for (timestep, fields) in solver.solve(interval, h):
    ...
    if (abs(round(timestep[1])-timestep[1])) < (0.1*h):
      ...
      # v_right.append(vs((5.0,0.0))[0])
      # cai_left.append(vs((0.0,0.0))[36])
      # cai_right.append(vs((5.0,0.0))[36])
      i=i+1
      print i
del hdf_v, hdf_cai
